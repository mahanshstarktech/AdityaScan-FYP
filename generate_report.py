...
def get_today():
    if "REPORT_DATE" in os.environ:
        return datetime.datetime.strptime(os.environ["REPORT_DATE"], "%Y-%m-%d").date()
    return datetime.date.today()

def get_repo_info():
    """Extracts the exact repository name and branch reliably in GitHub Codespaces."""
    repo_name = "AdityaScan-FYP"
    branch_name = "main"
    try:
...
def get_git_metrics(interval="weekly"):
    ...
    if interval == "weekly":
        ...
        if "REPORT_DATE" in os.environ:
            git_args.append(f"--until={today.strftime('%Y-%m-%d')} 23:59:59")
    ...
    for line in raw_output.strip().split('\n'):
        ...
        if line.startswith('COMMIT|||'):
            ...
            try:
                timeline_activity[current_date_str][current_author] += 1
            except Exception:
                pass
...
def create_charts(students, timeline_activity, interval):
    """Generates visual workload and trend charts with strict chronological order and distinct lines."""
    fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(11, 3.8))
    authors = list(students.keys())
    sorted_dates = sorted(timeline_activity.keys())

    # 1. Timeline Line Chart
    if sorted_dates and authors:
        labels = [datetime.datetime.strptime(d, "%Y-%m-%d").strftime("%b %d (%a)") for d in sorted_dates]
        color_map = {
            'Mahansh Gaur': ('#2CA02C', 'o', '-'),    # Green, circle
            'Somya Roy': ('#1F77B4', 's', '-'),       # Blue, square
            'Sanskriti Raj': ('#FF7F0E', '^', '-'),   # Orange, triangle
        }
        for author in authors:
            counts = [timeline_activity[d].get(author, 0) for d in sorted_dates]
            color, marker, ls = color_map.get(author, ('#7F7F7F', 'd', '-'))
            ax1.plot(labels, counts, marker=marker, linewidth=2.2, linestyle=ls, color=color, label=author, alpha=0.9)
    ...
    if authors:
        ...
        colors_list = ['#2CA02C', '#1F77B4', '#FF7F0E', '#76B7B2', '#59A14F']
...
def generate_pdf(interval="weekly"):
    ...
    date_stamp = get_today().strftime("%Y-%m-%d")
    ...
    story.append(Paragraph(f"<b>Evaluation Window:</b> {scope_title} &nbsp;|&nbsp; <b>Generated On:</b> {get_today().strftime('%B %d, %Y')}", meta_style))
